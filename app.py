...
class App:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.type == pygame.KEYDOWN and event.key == pygame.K_RETURN:
                    self.rules = False
            elif self.wait_enter ==  True and self.rules == False:
                if event.type == pygame.KEYDOWN and event.key == pygame.K_RETURN:
                    self.wait_enter = False
                    self.ready = False
            elif self.rules == False:   
    ...
    def update(self):
        ...
        self.need_letter = []
        if self.score[self.turn] >= STOPPER:
    ...
    def draw(self):
        ...
        if self.rules == True:
            ...
            self.draw_words( 'Вы играете в игру Эрудит!' , 600, 350)
        if self.game_over == True:
            pygame.draw.rect(self.screen, (255, 255, 255), (400, 300, 300, 130))
            pygame.draw.rect(self.screen, BLACK, (400, 300, 300, 130), 2)
            if self.winner == 0:
                self.draw_words('Игра окончена, Вы победили!', 550, 365)
                self.draw_words('Нажмите Enter, чтобы начать новую игру', 550, 380)
            else:
                self.draw_words('Игра окончена, победил компьютер!', 550, 365)
                self.draw_words('Нажмите Enter, чтобы начать новую игру', 550, 380)
        ...
        for i in range(15):
            for j in range(15):
                for k in lst: 
                    ...
                    can_put_down = False
                    for q in range(k):        
                        if j + q < 15 and self.matrix[i][j + q] != ' ':
                            can_put_right = True
                        if i + q < 15 and self.matrix[i + q][j] != ' ':
                            can_put_down = True
                    if can_put_right == True:
                        self.comp_dir = (0, 1)
                        self.comp_pos = (i, j)
                        for word in self.comp_dict[k]:
                            self.comp_word = word
                            if word in self.set_dict and self.check_pos():
                                self.all_words.append((self.comp_word.upper(), self.comp_pos, self.comp_dir))
                                return
                    if can_put_down == True:
                        self.comp_dir = (1, 0)
                        self.comp_pos = (i, j)
                        for word in self.comp_dict[k]:
                            self.comp_word = word
